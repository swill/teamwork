...
type GetTimeEntriesOps struct {
	...
	Page int `param:"page"`
	// The start date to retrieve from (eg: "YYYYMMDD")
	FromDate string `param:"fromdate"`
	// The start time only if fromdate is passed (eg: "HH:MM")
	FromTime string `param:"fromtime"`
	// The end date to retrieve to (eg: "YYYYMMDD")
	ToDate string `param:"todate"`
	// The end time only if todate is passed (eg: "HH:MM")
	ToTime string `param:"totime"`
	// One of the following: date, user, task, tasklist, project, company, dateupdated (Default: date)
	SortBy string `param:"sortby"`
	// The order to sort the returned data (eg: "ASC" or "DESC")
	SortOrder string `param:"sortorder"`
	// Return time logs for a specific user only
	UserID string `param:"userId"`
	// Filter the Time Entries to those that are Billable or Not Billable (eg: "billable" or "nonbillable").
	BillableType string `param:"billableType"`
	// filter the time entries to those that have been Invoiced or not (eg: "invoiced" or "noninvoiced").
	InvoicedType string `param:"invoicedType"`
	// Filter the time entries to those in Active projects, Archived projects or All projects (eg: "all", "active", "archived").
	ProjectType string `param:"projectType"`
	// Filter time entries to include deleted time sheet entries or not (eg: true or false).
